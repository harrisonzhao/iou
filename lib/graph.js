  ...
  if (source.worth + value > 0 && source.edges[sink_id] < 0) {
    ...
    for (var sourceEdge in source.edges) {
      if (source.edges.hasOwnProperty(sourceEdge)) {
        ...
        modifyEdge(graph, sink_id, sourceEdge, change);
        sourceVal += change;
  ...
  if (sink.worth - value < 0 && sink.edges[source_id] > 0) {
    ...
    for (var sinkEdge in sink.edges) {
      if (sink.edges.hasOwnProperty(sinkEdge)) {
        ...
        modifyEdge(graph, source_id, sinkEdge, -change);
        if (sinkVal === 0) break;
      }
    }
  }
...
var graph = {
  ...
  addTransaction : function(graph, transaction, source, sink) {
    ...
    reduceGraph(graph, source.user_id, sink.user_id, transaction.value);
    return true;
  }
